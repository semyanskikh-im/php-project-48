...
function differ($array1, $array2, $depth = 0)
{
    ...
    $result = array_map(function($key) use ($array1, $array2, $depth) {
        ...
        if (array_key_exists($key, $array1) && (!array_key_exists($key, $array2))) {
            $diff[] = ['depth' => $depth, 'status' => '-', 'children' => [$key => $array1[$key]]];
            return $diff;
        } 
        //если ключ есть только во втором массиве
        if (!array_key_exists($key, $array1) && (array_key_exists($key, $array2))) {
            $diff[] = ['depth' => $depth, 'status' => '+', 'children' => [$key => $array2[$key]]];
            return $diff;
        } 
          ...
          // если одно или второе значение не является массивом, то просто записываем разность
            ...
            if (is_array($array1[$key]) && is_array($array2[$key])) {
                ...
                if (array_is_list($array1[$key]) && array_is_list($array2[$key])) {
                    ...
                    return $diff[] = ['depth' => $depth, 'status' => ' ', 'children' => [$key => differ($array1[$key], $array2[$key], $depth + 1)]];
